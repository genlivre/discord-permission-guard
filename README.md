...
- **返信忘れ監視（Reply Monitor）**
  ...
  - **疎遠チャンネル**: 設定日数（例: 14日）以上「人間のやり取り」が無いチャンネルを未返信チェックページに「しばらくやり取りのないチャンネル」として表示（最近連絡を取っていないアーティストの検知。Discord 通知には載せない。空欄で無効。「不問」中・監視除外のチャンネルは表示しない）
- **定期実行 (Cron)**
...
チャンネルのワンクリック監視除外、Discord のメッセージ/チャンネルへのジャンプ、
チャンネル単位の「不問にする」（対応不要のまま放置してOK。新しいメッセージが来たら
自動で通知対象に復活）ができます。
...
| `POST /admin/api/reply-baseline` | 基準日時の設定/クリア（それより前に終わっている会話を一括不問化） |
| `POST /admin/api/reply-dismiss` | チャンネル単位の「不問」設定/解除（新着メッセージで自動失効し再通知） |
| `GET /admin` | 管理画面 |
...
5. 1 回の実行で Discord API を呼ぶ回数に**ギルドごとの上限**（既定 40。
   `REPLY_API_BUDGET_PER_GUILD` で変更可、Paid プランなら 250 程度を推奨）を設けており、
   超えたチャンネルは前回状態のまま次回実行へ持ち越す（Cloudflare Workers の
   サブリクエスト上限対策）。ギルドごとに独立させているため、チャンネル数の多い
   ギルドが後続ギルドの処理を妨げることはない
